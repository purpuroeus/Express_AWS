...
AWS.config.update({             // aws 설정 정보
  ...
  endpoint: "https://dynamodb.us-east-1.amazonaws.com"      // endpoint     프로토콜://서비스명.지역코드.amazonaws.com
...
function consoleErr(err, data){
    if(err){
        console.error("테이블 생성 불가, Error JSON :", JSON.stringify(err, null, 2));
    }
    else{
        console.log("테이블 생성 성공. 테이블 정보 JSON :", JSON.stringify(err, null, 2));
    }
}